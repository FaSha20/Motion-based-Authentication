...
## پاسخ به سوالات: 
### 1- با استفاده از ابزار perfetto از دستگاه داده جمع آوری کنید و عملیات Trace و Profile را انجام دهید و به سوالات زیر پاسخ دهید: 
### - از وقتی که درخواست خواندن داده به یک سنسور داده شده تا گرفتن داده چه اتفاقاتی در سطح سیستم عامل افتاده و چقدر زمان سپری شده است؟
<br/><br/>
درخواست لایه‌ی اپلیکیشن:
<br/><br/>
اپلیکیشن متد sensorManager.registerListener(listener, sensor, SensorManager.SENSOR_DELAY_NORMAL); را فراخوانی می‌کند./ 
زمان: ناچیز (چند میکروثانیه).
<br/><br/>
لایه‌ی فریمورک:
<br/><br/>
چارچوب اندروید درخواست را مدیریت کرده و با سرویس سنسور از طریق ارتباط بین فرآیندی (IPC) ارتباط برقرار می‌کند. /
زمان: معمولاً حدود ۱-۲ میلی‌ثانیه برای IPC.
<br/><br/>
سرویس سنسور:
<br/><br/>
سرویس سنسور درخواست را پردازش کرده و آن را به HAL سنسور ارسال می‌کند. /
زمان: تقریباً ۱ میلی‌ثانیه.
<br/><br/>
لایه‌ی سخت‌افزار انتزاعی (HAL):
<br/><br/>
هال با درایور سنسور در هسته برای پیکربندی شتاب‌سنج ارتباط برقرار می‌کند. /
زمان: حدود ۱-۲ میلی‌ثانیه.
<br/><br/>
لایه‌ی کرنل:
<br/><br/>
درایور سنسور، شتاب‌سنج را برای شروع جمع‌آوری داده‌ها پیکربندی می‌کند. /
زمان: ۱-۲ میلی‌ثانیه.
<br/><br/>
سخت‌افزار سنسور:
<br/><br/>
شتاب‌سنج شروع به اندازه‌گیری و گزارش داده‌ها در نرخ پیکربندی شده (هر ۲۰۰ میلی‌ثانیه برای SENSOR_DELAY_NORMAL) می‌کند./ 
زمان: سنسور هر ۲۰۰ میلی‌ثانیه داده‌ها را گزارش می‌دهد.
<br/><br/>
انتقال داده‌ها:
<br/><br/>
داده‌های سنسور از سخت‌افزار سنسور به درایور هسته، سپس به HAL، سرویس سنسور و در نهایت به اپلیکیشن ارسال می‌شود./ 
زمان: ۱-۲ میلی‌ثانیه برای انتقال داده‌ها از هر لایه.
<br/><br/>
دریافت داده‌ها در لایه‌ی اپلیکیشن:
<br/><br/>

اپلیکیشن داده‌های سنسور را از طریق متد بازخورد ثبت شده دریافت می‌کند (مثلاً onSensorChanged(SensorEvent event))./ 
زمان: ناچیز (فراخوانی متد بازخورد تقریباً فوری است).
<br/><br/>
### - زمان بین خواندن دو داده متوالی از سنسور در Perfetto را با دوره نمونه برداری که در کد خود پیکربندی نموده اید، مقایسه کنید.
<br/><br/>
در حالتی که نرخ نمونه برداری را برابر SENSOR_DELAY_NORMAL قرار دادیم ، این نرخ تقریبا برابر است.

<br/><br/>
### - آیا در فراخوانی های سیستمی، تعارضی (انتظار مشغول یک Thread تا زمانی که Thread دیگر کار خود را تمام کند) بین پردازش ها (مثال استفاده از کتابخانه مربوط به گرافیک) و بروزرسانی سنسورها وجود دارد؟ پاسخ خود را توجیه کنید.
<br/><br/>
در برخی از سناریوها، ممکن است فراخوانی‌های سیستمی مانند بروزرسانی گرافیک (که نیاز به پردازش سنگین و زمان‌بر هستند) تاخیرهای زمانی زیادی ایجاد کنند. این تاخیرها می‌توانند باعث ایجاد تعارض در زمانی که باید داده‌های سنسوری خوانده شوند و وظایف مرتبط با رابط کاربری انجام شود، شوند. به عنوان مثال، اگر برنامه‌ای در حال اجرا باشد که نیاز به نمایش یک انیمیشن گرافیکی داشته باشد و هم‌زمان از داده‌های سنسوری برای کنترل انیمیشن استفاده کند، ممکن است بروزرسانی گرافیکی تاخیرهای زمانی ایجاد کند که باعث تاخیر در خواندن داده‌های سنسوری شود، و این ممکن است باعث تأخیر در واکنش برنامه به ورودی‌های کاربری یا ایجاد اختلال در تجربه کاربری شود. برای جلوگیری از این مسائل، می‌توانید از روش‌هایی مانند استفاده از ترد های جداگانه برای پردازش داده‌های سنسوری، استفاده از مدیریت منابع منطقی (مانند ThreadPool) برای کنترل پردازش‌های زمان‌بر استفاده کرد.
<br/><br/>
### - مدت زمان لازم برای پردازش داده سنسورها را با زمان سایر پردازش های CPU مقایسه نمایید.
<br/><br/>
پردازش داده‌های سنسور: حدود 5-10 میلی‌ثانیه (به علاوه‌ی زمان نمونه‌برداری سنسور مانند 200 میلی‌ثانیه برای SENSOR_DELAY_NORMAL).
<br/><br/>
سوئیچینگ کانتکست: حدود 1-10 میکروثانیه (خیلی سریع‌تر از پردازش سنسور).
<br/><br/>
فراخوانی سیستم: حدود 0.1-1 میکروثانیه (خیلی سریع‌تر از پردازش سنسور).
<br/><br/>
پردازش I/O: از چند میکروثانیه تا میلی‌ثانیه (بسته به نوع I/O، ممکن است نزدیک به پردازش سنسور باشد).
<br/><br/>
محاسبات ریاضی پیچیده: بستگی به پیچیدگی، معمولاً در حدود میکروثانیه تا میلی‌ثانیه (ممکن است مشابه یا سریع‌تر از پردازش سنسور باشد).
<br/><br/>
پردازش داده‌های سنسور معمولاً در مقایسه با بسیاری از عملیات‌های سریع CPU زمان بیشتری می‌برد، اما این زمان‌ها همچنان نسبتاً کوتاه هستند و به گونه‌ای طراحی شده‌اند که تأثیر قابل توجهی بر عملکرد کلی سیستم نداشته باشند. این تفاوت‌ها نشان می‌دهد که پردازش داده‌های سنسور نسبت به پردازش‌های بسیار سریع CPU مانند سوئیچینگ کانتکست یا فراخوانی سیستم، زمان بیشتری می‌برد، اما همچنان در محدوده‌ای است که عملکرد اپلیکیشن‌ها را به‌طور جدی مختل نمی‌کند.
<br/><br/>

### 2- بهترین دوره تناوب برای خواندن مقادیر سنسور شتاب سنج )(و ژیروسکوپ) چه مقدار است؟ با استدلال توجیه شود.
<br/><br/>
مقادیر زیر را می توانستیم به عنوان نرخ نمونه برداری سنسور ها استفاده کنیم:
<br/><br/>
SENSOR_DELAY_NORMAL: 200,000 microseconds (5 Hz)
<br/><br/>
SENSOR_DELAY_UI: 60,000 microseconds (16.67 Hz)
<br/><br/>
SENSOR_DELAY_GAME: 20,000 microseconds (50 Hz)
<br/><br/>
SENSOR_DELAY_FASTEST: 0 microseconds (as fast as possible)
<br/><br/>
![Screenshot (532)](https://github.com/FaSha20/Motion-based-Authentication/assets/90645162/e74c1a74-1eef-4868-97ae-a9125e2caf81)
<br/><br/>
برای مثال در این آنالیز پرفتو  نرخ نمونه برداری ست شده روی حالت SENSOR_DELAY_GAME نشان داده شده است که مقداری تقریبا برابر با 20 میلی ثانیه و یک دهم مقدار حالت قبل دارد. و به طور کلی در کاربردهایی که نیاز به پاسخ‌دهی سریع دارند، این نرخ نمونه‌برداری بالا باعث کاهش تأخیر و افزایش دقت می‌شود. مثلاً در بازی‌های واقعیت مجازی، تأخیر حتی چند میلی‌ثانیه می‌تواند تجربه کاربری را تحت تأثیر قرار دهد پس این نرخ نمونه برداری مورد نیاز است.
اما در کاربرد هایی مثل کاربرد ما که تاخیر تاثیر چندانی روی عملکرد ندارد و همچنین باتری محدود است، نرخ نمونه‌برداری پایین‌تر باعث صرفه‌جویی در مصرف انرژی می‌شود و از نویزی که نمونه برداری مداوم می تواند ایجاد کند هم جلوگیری می کند، بدون اینکه تأثیر زیادی بر عملکرد داشته باشد. مثلاً در دستگاه‌های embeded حفظ عمر باتری اهمیت بالایی دارد. 
<br/><br/>
![Screenshot (534)](https://github.com/FaSha20/Motion-based-Authentication/assets/90645162/4defbb9d-bf61-4ab5-868b-9eeccf2a3748)
<br/><br/>
این تصویر نرخ نمونه برداری ست شده برای سنسور ها در حالت SENSOR_DELAY_NORMAL و مقایسه آن با نرخ واقعی کارکرد سنسور را نشان می دهد که می بینیم هر دو تقریبا برابر 200 میلی ثانیه هستند. همچنین با آزمون و خطای نرخ های مختلف می بینیم که این نرخ یافته های مناسب تری را برای ما دریافت می کند.
<br/><br/>
### 3- در مورد سنسور های based-hardware و based-software تحقیق نمایید و هر یک را تشریح نمایید. هر کدام از سنسورهای مورد استفاده در این تمرین در کدام دسته قرار میگیرند؟ 
<br/><br/>
سنسورهای harware based به صورت فیزیکی در دستگاه ها وجود دارند و مستقیما با محیط فیزیکی در تعامل هستند. این سنسورها داده خام را از محیط جمع آوری کرده و برای پردازش به سیستم ارسال میکنند. ژیروسکوپ، شتاب سنج، حسگر نور و... از این جنس سنسورها هستند که در این پروژه نیز از دو مورد آنها استفاده کردیم.
<br/><br/>
سنسورهای software based براساس ترکیب و پردازش داده‌های سنسورهای hardware based و الگوریتم‌های نرم‌افزاری عمل می‌کنند. این سنسورها به صورت مجازی هستند و از داده‌های جمع‌آوری شده توسط سنسورهای فیزیکی برای ارائه اطلاعات بیشتر یا کاربردهای جدید استفاده می‌کنند. ما در این پروژه تعدادی از این دست سنسورها را پیاده سازی کرده ایم. مانند سنسور محاسبه جهت که از داده های ژیروسکوپ و شتاب سنج برای تشخیص جهت گیری دستگاه استفاده میکرد.

<br/><br/>
### 4- چه تفاوتی بین تعریف سنسور به صورت up-wake و up-wake-non وجود دارد؟ ضمن تشریح مزایا و معایب هر کدام، مشخص کنید که انجام این کار تاثیری بر نحوه دریافت بروزرسانی سنسورها و نتیجه تشخیص مسیر دارد؟
<br/><br/>
سنسورهای wake-up و non wake-up دو نوع از سنسورها هستند که تفاوت‌های عمده‌ای در نحوه عملکرد و کاربردهای خود دارند. 
<br/><br/>
سنسورهای wake-up به گونه ای طراحی شده اند که ی‌توانند دستگاه را از حالت خواب (sleep mode) بیدار کنند یا توجه پردازنده را جلب کنند تا عملیات خاصی انجام دهد. این ویژگی به ویژه در سناریوهایی که نیاز به پاسخ سریع به تغییرات محیطی دارند، مفید است. از مزایای این سنسورها میتوان به 1. پاسخ دهی سریع (این سنسورها می‌توانند بلافاصله پس از تشخیص یک رویداد، دستگاه را بیدار کنند و پردازش لازم را انجام دهند.) 2. صرفه جویی در انرژی (به دلیل بیدار شدن دستگاه تنها در مواقع ضروری، مصرف انرژی کاهش می‌یابد زیرا دستگاه بیشتر اوقات در حالت کم‌مصرف (sleep mode) قرار دارد.) و 3. کارایی بالا در سناریوهای حساس (در برنامه‌هایی مانند تشخیص حرکت، امنیت و نظارت، که نیاز به پاسخ سریع دارند، این سنسورها بسیار مفید هستند.) اشاره کرد. اما این سنوسورها پیچیدگی بالاتری دارند و به دلیل برخورداری از تکنولوژی بالاتر و نیاز به هماهنگی با سیستم عامل و سخت افزار، هزینه تولید و استفاده بالاتری دارند. 
<br/><br/>
...
<br/><br/>
سنسورهای Wake-Up به دلیل توانایی بیدار کردن دستگاه و اجرای پردازش‌ها در لحظه، می‌توانند بروزرسانی‌ها و تشخیص‌های فوری و دقیق‌تری انجام دهند. این ویژگی باعث می‌شود که در کاربردهای حساس و فوری مثل امنیت و نظارت بسیار مؤثر باشند.
در سنسورهای Non Wake-Up اما بروزرسانی‌ها و تشخیص‌های این سنسورها ممکن است با تأخیر انجام شود زیرا نیاز به فعال بودن دستگاه دارند. این سنسورها برای کاربردهایی که زمان پاسخ‌دهی سریع ضروری نیست مناسب‌تر هستند. 
در این پروژه چون ما با فشردن دکمه start capture به سیستم اطلاع میدادیم که میخواهیم path جدیدی را ثبت کنیم، نیاز به سنسور wake up وجود نداشت و با سنسورهای non wake-up نیز میتوان این کاربری را با هزینه کمتر ایجاد کرد. اما اگر به عنوان یک سیستم قفل تلفن همراه در نظر بگیریم که هر لحظه ممکن است کاربر بخواهد با حرکت دادن دستگاهش در جهت تعیین شده، قفل را باز کند، در این صورت به سنسورهای wake-up نیاز داریم که هزینه بیشتری هم به طبع دارد. 
